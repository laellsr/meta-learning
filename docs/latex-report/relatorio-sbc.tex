...
\begin{itemize}
  ...
  \item Validação em quatro cenários experimentais: (i) dados controlados; (ii) 144 datasets reais do OpenML; (iii) 200 datasets reais de famílias diferentes; e (iv) 200 datasets de famílias diferentes com 78 meta-features extraídas pelo PyMFE;
...
\begin{enumerate}
  \item \textbf{Baseline} (sem escalonamento): $\tilde{y} = y$. Serve como grupo de controle, mantendo os valores de desempenho originais (F1-macro) sem qualquer modificação;
  \item \textbf{StandardScaler} (Z-score): $\tilde{y} = (y - \mu) / \sigma$. Onde $\mu$ é a média e $\sigma$ o desvio padrão. Centraliza os dados em torno de zero com variância unitária, auxiliando na convergência de métodos baseados em gradiente;
  \item \textbf{MinMaxScaler}: $\tilde{y} = (y - y_{\min}) / (y_{\max} - y_{\min})$. Mapeia linearmente os valores para o intervalo $[0, 1]$, preservando a forma da distribuição original e a proporção das distâncias entre os pontos;
  \item \textbf{RobustScaler}: $\tilde{y} = (y - \text{mediana}) / \text{IQR}$. Onde $\text{IQR}$ é o intervalo interquartil ($Q_3 - Q_1$). Utiliza métricas robustas à presença de outliers (mediana e quartis), evitando que desempenhos anomalamente baixos ou altos distorçam a escala;
  \item \textbf{QuantileTransformer}: $\tilde{y} = \Phi^{-1}\!\left( \frac{\text{rank}(y)}{n+1} \right)$. Onde $\Phi^{-1}$ é a função inversa da distribuição normal cumulativa e $n$ é o número de amostras. Mapeia os dados de forma não-linear para uma distribuição normal, suavizando variações extremas, mas alterando as distâncias relativas entre os ranks.
\end{enumerate}
...
Este estudo analisou como o escalonamento do meta-alvo influencia o desempenho de sistemas de meta-aprendizagem na tarefa de selecionar algoritmos. Para isso, realizaram-se quatro experimentos: 50 datasets em ambiente controlado, 144 datasets reais (etapa inicial), 200 datasets reais de famílias diferentes (etapa expandida) e, finalmente, 200 datasets reais de famílias diferentes utilizando 78 meta-features extraídas automaticamente pelo PyMFE.
...
O \textbf{QuantileTransformer deve ser evitado se o objetivo principal for otimizar a qualidade das recomendações} (Acc@1), uma vez que tende a degradar a ordenação. No entanto, ele se destaca como a escolha mais eficaz na presença de dados altamente desbalanceados, além de ter melhorado a correlação de Spearman do MLP no Experimento 4 com PyMFE.
